...
```{r}
# Second model: average rating + movie effect. The test set is used only to create a vector of predictions of the same size as the rating vector of the test set

y_hat = mean(train_set$rating) + (test_set %>% left_join(train_set, by='movieId') %>% .$m_i)
```
